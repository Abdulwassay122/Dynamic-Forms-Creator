...
export default function ViewForm() {
    ...
    const onSubmit = async () => {
        try {
            ...
            // Check If response with this email Already Exixsts
            ...
            if (upload.status === 201) {
                ...
                sessionStorage.removeItem('formPartialData');
                navigate("/thanks");
                reset();
            }
    ...
    return (
        <section className="min-h-screen lg:px-60 md:px-20 sm:px-10 px-5 py-20 flex flex-col gap-10 bg-gray-100 bg-gradient-l-r from-gray-100 to-gray-200 ">
            ...
            {!loading && formData?.sections.length ? <div className="relative bg-white rounded-md shadow-lg"><div className="px-10 flex  flex-col gap-4 items-center">
                ...
                {formData?.sections.length > 1 && <div className="flex items-center pt-18 w-full justify-between">
                    {formData.sections.map((_: any, e: number) => (<>
                        ...
                        <div onClick={() => handleIndex(e)} className={`cursor-pointer h-10 min-w-10 flex justify-center items-center rounded-[100%] text-white ${currentSectionIndex >= e ? "bg-gray-800" : "bg-gray-200"} `}>{e + 1}</div>
                        <div className={`rounded-full border-solid ${currentSectionIndex >= e ? 'border-gray-800' : 'border-gray-200'}  w-full border-2`}></div>
                    </>))}
                </div>}
            </div>
                    ...
                    {formData?.sections.length > 0 && (<div className={`pb-5  flex flex-col gap-5 ${formData?.sections.length > 1 ? "pt-5" : "pt-10"}`}>
                        ...
                        <div className="grid sm:grid-cols-2 gap-5 px-10">
                            {formData.sections[currentSectionIndex]?.fields.map((ele: any, id: number) => (
                                <div className={`${ele.length === "full" ? "sm:col-span-2" : "sm:col-span-1"
                                    } w-full flex flex-col justify-center`}>
                                    <Label key={id} className="block mb-1 capitalize text-sm"> {ele.label} <span className="text-red-600">*</span></Label>
                                    {ele.type === "text" && (
                                        <Controller
                                            name={ele.id}
                                            control={control}
